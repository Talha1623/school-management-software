...
namespace App\Http\Controllers;
    ...
    class StaffSalaryReportController extends Controller
    {
        ...
        {
            $filterCampus = $this->normalizeFilter($request->get('filter_campus'));
            $filterYear = trim((string) $request->get('filter_year', date('Y')));
            ...
            $monthNames = $this->monthNumberNameMap();
            ...
            foreach ($allStaff as $staff) {
                $salaries = Salary::where('staff_id', $staff->id)
                    ...
                    ->get()
                    ->keyBy(fn (Salary $salary) => $this->normalizeSalaryMonthKey($salary->salary_month));
                ...
                foreach ($monthNames as $monthNum => $monthName) {
                    ...
                    $monthlyData[] = [
                        ...
                        'basic_salary' => $salary ? (float) $salary->basic : 0,
                ...
                $staffReports->push([
                    ...
                    'totals' => [
                        'present' => collect($monthlyData)->sum('present'),
                        'absent' => collect($monthlyData)->sum('absent'),
                        'late' => collect($monthlyData)->sum('late'),
                        'leaves' => collect($monthlyData)->sum('leaves'),
                        'holidays' => collect($monthlyData)->sum('holidays'),
                        'sundays' => collect($monthlyData)->sum('sundays'),
                        'basic_salary' => collect($monthlyData)->sum('basic_salary'),
                        'salary_generated' => collect($monthlyData)->sum('salary_generated'),
                        'amount_paid' => collect($monthlyData)->sum('amount_paid'),
                        'loan_repayment' => collect($monthlyData)->sum('loan_repayment'),
                    ],
                ]);
            }
        ...
        private function monthNumberNameMap(): array
        {
            return [
                '01' => 'January',
                '02' => 'February',
        ...
        private function normalizeSalaryMonthKey(?string $month): string
        {
