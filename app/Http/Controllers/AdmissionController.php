...
class AdmissionController extends Controller
{
    ...
    /**
     * Student app login password — same rules as single admit (store).
     */
    private function resolveStudentLoginPassword(array $studentData): string
    {
        $fromBFormOrCsvPassword = trim((string) ($studentData['b_form_number'] ?? ''));
        if ($fromBFormOrCsvPassword !== '') {
            return $fromBFormOrCsvPassword;
        }

        $explicit = trim((string) ($studentData['student_password'] ?? ''));
        if ($explicit !== '') {
            return $explicit;
        }
    ...
    public function store(Request $request): JsonResponse|RedirectResponse
    {
        ...
        // Hash B-Form Number as password if provided (same default as bulk: "student")
        $studentData['password'] = $this->resolveStudentLoginPassword($studentData);
        ...
        if (empty($studentData['student_code']) && !empty($validated['campus'])) {
            ...
            while (
                Student::where('student_code', $studentData['student_code'])->exists()
                || StudentPayment::where('student_code', $studentData['student_code'])->exists()
            ) {
            ...
            $code = trim((string) $studentData['student_code']);
            if (Student::where('student_code', $code)->exists()) {
                $message = 'This student code is already registered. Please refresh the page to get a new code.';
                if ($request->ajax() || $request->wantsJson()) {
                    return response()->json([
                        'success' => false,
                        'message' => $message,
            ...
            if (StudentPayment::where('student_code', $code)->exists()) {
                $message = 'Student code already has payment history. Please refresh the page for a new code.';
                if ($request->ajax() || $request->wantsJson()) {
                    return response()->json([
                        'success' => false,
                        'message' => $message,
        ...
        try {
            $result = DB::transaction(function () use (
                ...
                // Monthly fee is NOT auto-generated on admission — use Accounting → Generate Monthly Fee.
                $this->removeAutoGeneratedMonthlyFees($student->fresh());
                ...
                return compact('student', 'printUrl', 'feeVoucherUrl', 'successMessage');
            });
        } catch (\Throwable $e) {
            \Log::error('Admission store failed: '.$e->getMessage(), [
        ...
        $feeVoucherUrl = $result['feeVoucherUrl'];
    ...
    private function removeAutoGeneratedTransportFees(Student $student): void
    {
        if (empty($student->student_code)) {
            return;
        }
        ...
        StudentPayment::where('student_code', $student->student_code)
            ->where('method', 'Generated')
            ->where(function ($query) {
                $query->where('payment_title', 'like', 'Transport Fee%')
                    ->orWhere('payment_title', 'like', 'transport fee%');
            })
            ->delete();
    }
        ...
        try {
            ...
            if ($inputMethod === 'csv') {
                ...
                if (in_array($fileExtension, ['xlsx', 'xls'])) {
                    try {
                        $rows = SpreadsheetReader::toArray($file);
                    } catch (\Exception $e) {
                        return redirect()
                ...
                foreach ($students as $index => $studentData) {
                    try {
                        ...
                        $student = Student::create([
                            ...
                            'b_form_number' => !empty($studentData['b_form_number'])
                                ? trim((string) $studentData['b_form_number'])
                                : null,
                            'transport_route' => $studentData['transport_route'] ?? null,
                            'transport_fare' => $studentData['transport_fare'] ?? null,
                            'parent_account_id' => $parentAccountId,
                            'password' => $this->resolveStudentLoginPassword($studentData),
                        ]);
                        
                        // Handle arrears as pending/generated so it appears in Unpaid + Fee Voucher.
                        if (!empty($studentData['arrears']) && is_numeric($studentData['arrears']) && $studentData['arrears'] > 0) {
                            $this->createGeneratedFeeRecord(
                                $student,
                                'Arrears',
                                (float) $studentData['arrears'],
                                $student->admission_date ? (string) $student->admission_date : now()->format('Y-m-d'),
                                $this->resolveRecordingAccountantName()
                            );
                        }
                        $this->removeAutoGeneratedMonthlyFees($student);
                        $this->removeAutoGeneratedTransportFees($student);
