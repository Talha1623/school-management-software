...
class AdmissionController extends Controller
{
            ...
            $result = DB::transaction(function () use (
                ...
                if (! empty($student->student_code)) {
                    $generatedFeeBillingDate = $student->created_at
                        ? Carbon::parse($student->created_at)->toDateString()
                        : Carbon::now()->toDateString();
                    $this->createGeneratedMonthlyFeeIfApplicable($student->fresh(), $generatedFeeBillingDate, $accountantName);
                }
    ...
    /**
     * Generated monthly fee stub — required for Fee Payment / voucher listing.
     */
    private function createGeneratedMonthlyFeeIfApplicable(Student $student, ?string $dueDate, string $accountantName): void
    {
        $monthlyFee = (float) ($student->monthly_fee ?? 0);
        if ($monthlyFee <= 0.00001 || empty($student->student_code)) {
            return;
        }
        ...
        $billing = Carbon::parse($billingDate);
        $monthlyTitle = 'Monthly Fee - '.$billing->format('F').' '.$billing->format('Y');
