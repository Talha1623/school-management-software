...
class SalaryDecrementPercentageController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'absent_fees_decrement' => ['nullable', 'numeric', 'min:0', 'max:100'],
            'late_fees_decrement' => ['nullable', 'numeric', 'min:0', 'max:100'],
            'early_exit_fees_decrement' => ['nullable', 'numeric', 'min:0', 'max:100'],
        ...
        foreach ($staff as $member) {
            ...
            if ($currentSalary > 0) {
                ...
                // Only update salary - fees will only be updated if manual decrement percentages are provided
                $updateData = [
                    'salary' => round($newSalary, 2),
                ];
                
                // Decrement Absent Fees only if manual decrement percentage is provided
                if ($absentFeesDecrement !== null && $absentFeesDecrement > 0) {
                    ...
                    if ($currentAbsentFees !== null && $currentAbsentFees > 0) {
                        $newAbsentFees = $currentAbsentFees - ($currentAbsentFees * ($absentFeesDecrement / 100));
                        $newAbsentFees = max(0, $newAbsentFees);
                        $updateData['absent_fees'] = round($newAbsentFees, 2);
                    }
                }
                
                // Decrement Late Fees only if manual decrement percentage is provided
                if ($lateFeesDecrement !== null && $lateFeesDecrement > 0) {
                    ...
                    if ($currentLateFees !== null && $currentLateFees > 0) {
                        $newLateFees = $currentLateFees - ($currentLateFees * ($lateFeesDecrement / 100));
                        $newLateFees = max(0, $newLateFees);
                        $updateData['late_fees'] = round($newLateFees, 2);
                    }
                }
                
                // Decrement Early Exit Fees only if manual decrement percentage is provided
                if ($earlyExitFeesDecrement !== null && $earlyExitFeesDecrement > 0) {
                    ...
                    if ($currentEarlyExitFees !== null && $currentEarlyExitFees > 0) {
                        $newEarlyExitFees = $currentEarlyExitFees - ($currentEarlyExitFees * ($earlyExitFeesDecrement / 100));
