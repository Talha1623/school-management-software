...
<div class="print-container">
  ...
  <table>
    ...
    <tbody>
      @forelse($timetables as $i => $timetable)
        <tr>
          ...
          <td>
            ...
            @if(!empty($timetable->assigned_teacher))
              <div style="font-size:10px; color:#555;">Teacher: {{ $timetable->assigned_teacher }}</div>
            @endif
          </td>
      ...
      @empty
        <tr>
          <td colspan="8" class="center">No timetables found</td>
        </tr>
      @endforelse
    </tbody>
  </table>
  ...
  <div class="footer">
    <div><strong>Total:</strong> {{ $timetables->count() }}</div>
