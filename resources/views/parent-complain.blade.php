    ...
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3 mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="mb-0 fs-16 fw-semibold">Parent Complain</h4>
            </div>
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-3 p-3 rounded-8" style="background-color: #f8f9fa; border: 1px solid #e9ecef;">
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="d-flex gap-2">
                        <a href="{{ route('parent-complain.export', ['format' => 'excel']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn excel-btn">
                            <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">description</span>
                            <span>Excel</span>
                        </a>
                        <a href="{{ route('parent-complain.export', ['format' => 'csv']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn csv-btn">
                            <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">file_present</span>
                            <span>CSV</span>
                        </a>
                        <a href="{{ route('parent-complain.export', ['format' => 'pdf']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn pdf-btn" target="_blank">
            ...
            <div class="mb-2 p-2 rounded-8" style="background: linear-gradient(135deg, #003471 0%, #004a9f 100%);">
                <h5 class="mb-0 text-white fs-15 fw-semibold d-flex align-items-center gap-2">
                    ...
                    <span>Parent Complains List</span>
                </h5>
            </div>
            ...
            @if(request('search'))
                <div class="search-results-info">
                    ...
                    @if(isset($complains))
                        ({{ $complains->total() }} {{ Str::plural('result', $complains->total()) }} found)
                    @endif
                    <a href="{{ route('parent-complain') }}" class="text-decoration-none ms-2" style="color: #003471;">
            ...
            <div class="default-table-area" style="margin-top: 0;">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            @if(isset($complains) && $complains->count() > 0)
                                @forelse($complains as $complain)
                                    <tr>
                                        <td>{{ $loop->iteration + (($complains->currentPage() - 1) * $complains->perPage()) }}</td>
                                        <td>
                                            <strong class="text-primary">{{ $complain->parent_name ?? 'N/A' }}</strong>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span class="text-muted">
                                                ...
                                                {{ $complain->created_at ? $complain->created_at->format('d M Y') : 'N/A' }}
                                            </span>
                                        </td>
                                        <td>
                                            @php
                                ...
                                @empty
                                    <tr>
                                        <td colspan="9" class="text-center text-muted py-5">
                                            ...
                                            <p class="mt-2 mb-0">No complains found.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            @else
            ...
            @if(isset($complains) && $complains->hasPages())
                <div class="mt-3">
                    {{ $complains->links() }}
                </div>
            @endif
...
function printTable() {
    ...
    document.body.innerHTML = `
        <div style="padding: 20px;">
            <h3 style="text-align: center; margin-bottom: 20px; color: #003471;">Parent Complains List</h3>
            ${printContents}
        </div>
    `;
