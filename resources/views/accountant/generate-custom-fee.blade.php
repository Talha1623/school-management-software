...
@section('title', 'Generate Custom Fee - Accountant')
...
<div class="row">
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3">
            ...
            <form action="{{ route('accountant.generate-custom-fee.store') }}" method="POST" id="custom-fee-form">
                @csrf
...
<script>
const customFeeRestoreCampus = @json(old('campus', $defaultCampus ?? ''));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to load fee types by campus (must match Fee Type rows for this campus, plus global/null campus heads)
    function loadFeeTypesByCampus(campus) {
        ...
        fetch(`{{ route('accountant.custom-fee.get-fee-types-by-campus') }}?campus=${encodeURIComponent(campus)}&master_only=1`)
            .then(response => {
                if (!response.ok) {
    ...
    function loadStudents() {
        ...
        // Fetch students
        const url = `{{ route('accountant.custom-fee.get-students') }}?${params.toString()}`;
