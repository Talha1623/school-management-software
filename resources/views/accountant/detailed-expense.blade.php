    ...
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3 mb-4">
            ...
            @php
                ...
                if (empty($exportQuery['filter_campus']) && !empty($filterCampus)) {
                    $exportQuery['filter_campus'] = $filterCampus;
                }
                $showResults = request()->hasAny(['filter_campus', 'filter_month', 'filter_date', 'filter_year', 'filter_method'])
                    || !empty($defaultCampus);
            @endphp
            ...
            <form action="{{ route('accountant.detailed-expense') }}" method="GET" id="filterForm">
                <div class="row g-2 mb-3 align-items-end">
                    ...
                    <div class="col-md-2">
                        ...
                        <select class="form-select form-select-sm" id="filter_campus" name="filter_campus" style="height: 32px;" @if(!empty($defaultCampus) && $campuses->count() <= 1) disabled @endif>
                        ...
                        @if(!empty($defaultCampus) && $campuses->count() <= 1)
            ...
            <div class="mt-3">
                ...
                <div class="d-flex justify-content-end mb-2">
                    <div class="d-flex gap-2 flex-wrap">
                        <a href="{{ route('accountant.detailed-expense.export', ['format' => 'excel']) }}?{{ http_build_query($exportQuery) }}" class="btn btn-sm px-2 py-1 export-btn excel-btn">
                            <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">description</span>
                            <span>Excel</span>
                        </a>
                        <a href="{{ route('accountant.detailed-expense.export', ['format' => 'csv']) }}?{{ http_build_query($exportQuery) }}" class="btn btn-sm px-2 py-1 export-btn csv-btn">
                            <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">table_view</span>
                            <span>CSV</span>
                        </a>
                        <a href="{{ route('accountant.detailed-expense.export', ['format' => 'pdf']) }}?{{ http_build_query($exportQuery) }}" class="btn btn-sm px-2 py-1 export-btn pdf-btn" target="_blank">
                            <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">picture_as_pdf</span>
                            <span>PDF</span>
                        </a>
                        <a href="{{ route('accountant.detailed-expense.print') }}?{{ http_build_query(array_merge($exportQuery, ['auto_print' => 1])) }}" class="btn btn-sm px-2 py-1 export-btn print-btn" target="_blank">
                                        ...
                                        <div class="d-inline-flex align-items-center gap-1 flex-wrap">
                                            @if(($record['record_type'] ?? '') === 'expense' && !empty($record['expense_id']))
                                                <a class="btn btn-sm btn-primary px-2 py-1 d-inline-flex align-items-center gap-1"
                                                   href="{{ route('accountant.add-manage-expense.print', $record['expense_id']) }}"
                                                   target="_blank"
                                                   title="Print expense slip">
                                                    <span class="material-symbols-outlined" style="font-size: 14px;">print</span>
                                                    Slip
                                                </a>
                                                <form action="{{ route('accountant.add-manage-expense.destroy', $record['expense_id']) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this expense?');">
                                                    @csrf
                                                    @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const methodsUrl = @json(route('accountant.detailed-expense.get-methods-by-campus'));
    ...
    function loadMethods(selectedMethod = '') {
        ...
        } else if (@json($defaultCampus ?? null)) {
            params.append('filter_campus', @json($defaultCampus ?? null));
        }
