...
@section('title', 'Bulk Fee Payment - Accountant')
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select form-select-sm" id="filter_campus" style="height: 32px;">
                        ...
                        @foreach($campuses as $campus)
                            <option value="{{ $campus }}" {{ auth()->guard('accountant')->check() && auth()->guard('accountant')->user()->campus == $campus ? 'selected' : '' }}>{{ $campus }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-load classes and fee types if campus is already selected (e.g., from accountant's default campus)
