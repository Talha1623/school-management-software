...
@section('title', 'Generate Transport Fee - Accountant')

@section('content')
<div class="row">
    <div class="col-12">
        <div class="d-flex align-items-center mb-4">
            <h2 class="mb-0 fs-20 fw-semibold text-dark me-2">Generate Transport Fee</h2>
            <span class="material-symbols-outlined text-secondary" style="font-size: 20px;">directions_bus</span>
        </div>
        ...
        <div class="card bg-white border border-white rounded-10 p-3">
            ...
            <form action="{{ route('accountant.generate-transport-fee.store') }}" method="POST" id="transport-fee-form">
                @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadClassesForCampus() {
        ...
        fetch(`{{ route('accountant.transport-fee.get-classes-by-campus') }}?campus=${encodeURIComponent(campus)}`, {
            headers: {
    ...
    function loadStudents() {
        ...
        fetch(`{{ route('accountant.transport-fee.get-students-with-fee-status') }}?${params.toString()}`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
            }
    ...
    classSelect.addEventListener('change', function() {
        ...
        fetch(`{{ route('accountant.transport-fee.get-sections-by-class') }}?class=${encodeURIComponent(selectedClass)}&campus=${encodeURIComponent(campus)}`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
            }
    ...
    document.getElementById('transport-fee-form').addEventListener('reset', function() {
        setTimeout(function() {
            resetClassAndSection();
...
function resetForm() {
    ...
    document.getElementById('student-selection-section').style.display = 'none';
    document.getElementById('students-list').innerHTML = '';
    document.getElementById('transport-fee-form').reset();
}
