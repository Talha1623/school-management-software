...
@section('title', 'Fee Type / Fee Head - Accountant')
    ...
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3 mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="mb-0 fs-16 fw-semibold">Fee Type / Fee Head</h4>
                <button type="button" class="btn btn-sm py-2 px-3 d-inline-flex align-items-center gap-1 rounded-8 fee-type-add-btn" data-bs-toggle="modal" data-bs-target="#feeTypeModal" onclick="resetForm()">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-3 p-3 rounded-8" style="background-color: #f8f9fa; border: 1px solid #e9ecef;">
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="d-flex gap-2">
                        <a href="{{ route('accountant.fee-type.export', ['format' => 'excel']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn excel-btn">
                            <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">description</span>
                            <span>Excel</span>
                        </a>
                        <a href="{{ route('accountant.fee-type.export', ['format' => 'csv']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn csv-btn">
                            <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">file_present</span>
                            <span>CSV</span>
                        </a>
                        <a href="{{ route('accountant.fee-type.export', ['format' => 'pdf']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn pdf-btn" target="_blank">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="input-group input-group-sm search-input-group" style="width: 280px;">
                            ...
                            <input type="text" id="searchInput" class="form-control border-start-0 border-end-0" placeholder="Search fee types..." value="{{ request('search') }}" onkeypress="handleSearchKeyPress(event)" oninput="handleSearchInput(event)" style="padding: 4px 8px; font-size: 13px;">
                            @if(request('search'))
            ...
            @if(request('search'))
                <div class="search-results-info">
                    ...
                    @endif
                    <a href="{{ route('accountant.fee-type') }}" class="text-decoration-none ms-2" style="color: #003471;">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <div class="d-inline-flex gap-1">
                                                ...
                                                <form id="delete-form-{{ $feeType->id }}" action="{{ route('accountant.fee-type.destroy', $feeType) }}" method="POST" class="d-none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </div>
            ...
            @if(isset($feeTypes) && $feeTypes->hasPages())
                <div class="mt-3">
                    {{ $feeTypes->links() }}
                </div>
            @endif
...
<div class="modal fade" id="feeTypeModal" tabindex="-1" aria-labelledby="feeTypeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px; overflow: hidden;">
            ...
            <form id="feeTypeForm" method="POST" action="{{ route('accountant.fee-type.store') }}">
                @csrf
                <div id="methodField"></div>
                <div class="modal-body p-3">
                    <div class="row g-2">
                        <div class="col-md-6">
                            <label class="form-label mb-1 fs-12 fw-semibold" style="color: #003471;">Fee Name <span class="text-danger">*</span></label>
                            <div class="input-group input-group-sm fee-type-input-group">
                                <span class="input-group-text" style="background-color: #f0f4ff; border-color: #e0e7ff; color: #003471;">
                                    <span class="material-symbols-outlined" style="font-size: 15px;">title</span>
                                </span>
                                <input type="text" class="form-control fee-type-input" name="fee_name" id="fee_name" placeholder="Enter fee name" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                ...
                <div class="modal-footer p-3" style="background-color: #f8f9fa; border-top: 1px solid #e9ecef;">
                    ...
                    <button type="submit" class="btn btn-sm py-2 px-4 rounded-8 fee-type-submit-btn">
...
<style>
    /* Fee Type Form Styling */
    #feeTypeModal .fee-type-input-group {
        height: 36px;
        border-radius: 8px;
        overflow: hidden;
    ...
    #feeTypeModal .fee-type-input-group:focus-within {
        box-shadow: 0 0 0 3px rgba(0, 52, 113, 0.15);
        border-color: #003471;
    }
    
    #feeTypeModal .fee-type-input {
        height: 36px;
        font-size: 13px;
    ...
    #feeTypeModal .fee-type-input:focus {
        border-left-color: #003471;
        box-shadow: none;
        outline: none;
    }
    
    #feeTypeModal .input-group-text {
        height: 36px;
        padding: 0 0.75rem;
        display: flex;
        align-items: center;
    ...
    #feeTypeModal .fee-type-input-group:focus-within .input-group-text {
        background-color: #003471 !important;
        color: white !important;
        border-right-color: #003471;
    }
    
    #feeTypeModal .form-label {
        margin-bottom: 0.4rem;
        line-height: 1.3;
    }
    
    #feeTypeModal .fee-type-submit-btn {
    ...
    #feeTypeModal .fee-type-submit-btn:hover {
        background: linear-gradient(135deg, #004a9f 0%, #003471 100%);
        transform: translateY(-1px);
        box-shadow: 0 4px 12px rgba(0, 52, 113, 0.35);
    }
    
    #feeTypeModal .fee-type-submit-btn:active {
...
<script>
// Reset form when opening modal for new fee type
function resetForm() {
    document.getElementById('feeTypeForm').reset();
    document.getElementById('feeTypeForm').action = "{{ route('accountant.fee-type.store') }}";
...
// Edit fee type function
function editFeeType(id) {
    fetch(`{{ url('/accountant/fee-type') }}/${id}`)
        .then(response => response.json())
        .then(data => {
            document.getElementById('feeTypeForm').action = '{{ route('accountant.fee-type.update', ':id') }}'.replace(':id', id);
            ...
            document.getElementById('fee_name').value = data.fee_name || '';
            document.getElementById('campus').value = data.campus || '';
            
            new bootstrap.Modal(document.getElementById('feeTypeModal')).show();
        })
        .catch(error => {
            console.error('Error:', error);
            alert('Error loading fee type data');
        });
}
...
function printTable() {
    ...
    document.body.innerHTML = `
        <div style="padding: 20px;">
            <h3 style="text-align: center; margin-bottom: 20px; color: #003471;">Fee Types List</h3>
            ${printContents}
        </div>
    `;
