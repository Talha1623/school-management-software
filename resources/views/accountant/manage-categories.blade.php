...
@section('title', 'Manage Categories - Accountant')
    ...
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3 mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="mb-0 fs-16 fw-semibold">Manage Categories</h4>
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-3 p-3 rounded-8" style="background-color: #f8f9fa; border: 1px solid #e9ecef;">
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="d-flex gap-2">
                        <a href="{{ route('accountant.manage-categories.export', ['format' => 'excel']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn excel-btn">
                            <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">description</span>
                            <span>Excel</span>
                        </a>
                        <a href="{{ route('accountant.manage-categories.export', ['format' => 'pdf']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn pdf-btn" target="_blank">
            ...
            @if(request('search'))
                <div class="search-results-info">
                    ...
                    @if(isset($categories))
                        ({{ $categories->total() }} {{ $categories->total() == 1 ? 'result' : 'results' }} found)
                    @endif
                    <a href="{{ route('accountant.manage-categories') }}" class="text-decoration-none ms-2" style="color: #003471;">
                            ...
                            @if(isset($categories) && $categories->count() > 0)
                                @forelse($categories as $category)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="text-muted">{{ $category->description ? (strlen($category->description) > 50 ? substr($category->description, 0, 50) . '...' : $category->description) : 'N/A' }}</span>
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-end">
                                            <div class="d-inline-flex gap-1">
                                                <button type="button" class="btn btn-sm btn-primary px-2 py-0" title="Edit" onclick="editCategory({{ $category->id }}, '{{ addslashes($category->category_name) }}', '{{ addslashes($category->description ?? '') }}', '{{ addslashes($category->campus) }}')">
                                                ...
                                                <form id="delete-form-{{ $category->id }}" action="{{ route('accountant.manage-categories.destroy', $category->id) }}" method="POST" class="d-none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </div>
...
<div class="modal fade" id="categoryModal" tabindex="-1" aria-labelledby="categoryModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px; overflow: hidden;">
            ...
            <form id="categoryForm" method="POST" action="{{ route('accountant.manage-categories.store') }}">
                @csrf
                    ...
                    <div class="row g-2">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="input-group input-group-sm category-input-group">
                                ...
                                <textarea class="form-control category-input" name="description" id="description" placeholder="Enter description" rows="3" style="resize: none;"></textarea>
                            </div>
                        </div>
                        <div class="col-md-12">
...
function resetForm() {
    ...
    document.getElementById('categoryForm').action = "{{ route('accountant.manage-categories.store') }}";
...
function editCategory(id, categoryName, description, campus) {
    document.getElementById('category_name').value = categoryName;
    document.getElementById('description').value = description;
    document.getElementById('campus').value = campus;
    document.getElementById('categoryForm').action = "{{ url('accountant/manage-categories') }}/" + id;
