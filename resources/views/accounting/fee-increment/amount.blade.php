            ...
            <form id="feeIncrementAmountForm" method="POST" action="{{ route('accounting.fee-increment.amount.store') }}">
                ...
                <div class="payment-row mb-3 p-3 border rounded" style="background-color: #f8f9fa;">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-primary" id="loadStudentsBtn" onclick="loadStudentsForAmount()" disabled>
                            Load Students
                        </button>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadClassesByCampus(campus) {
        ...
        fetch(`{{ route('accounting.fee-increment.amount.get-classes-by-campus') }}?campus=${encodeURIComponent(campus)}`)
    ...
    function loadSectionsByClass(campus, className) {
        ...
        fetch(`{{ route('accounting.fee-increment.amount.get-sections-by-class') }}?${params.toString()}`)
            .then(response => response.json())
            .then(sections => {
...
function loadStudentsForAmount() {
    const campus = document.getElementById('campus').value || '';
    const className = document.getElementById('class').value || '';
    const section = document.getElementById('section').value || '';
    ...
    fetch(`{{ route('accounting.fee-increment.amount.get-students') }}?${params.toString()}`)
        .then(response => response.json())
        .then(data => {
...
function updateAmountPreview() {
    const increaseValue = parseFloat(document.getElementById('amount').value || 0);
    ...
    rows.forEach(row => {
        ...
        const newFee = currentFee + increaseValue;
