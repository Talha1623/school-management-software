            ...
            <form id="feeDecrementAmountForm" method="POST" action="{{ route('accounting.fee-document.decrement-amount.store') }}">
                            ...
                            <div class="input-group">
                                ...
                                <select class="form-select form-select-sm" name="accountant" id="accountant" style="height: 38px;">
                                    ...
                                    @foreach($accountants as $accountant)
                                        <option value="{{ $accountant->name }}" {{ ($defaultAccountant ?? '') === $accountant->name ? 'selected' : '' }}>
                                            {{ $accountant->name }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="payment-row mb-3 p-3 border rounded" style="background-color: #f8f9fa;">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-primary" id="loadStudentsBtn" onclick="loadStudentsForDecrementAmount()" disabled>
                            Load Students
                        </button>
                    </div>
...
function loadStudentsForDecrementAmount() {
    const campus = document.getElementById('campus').value || '';
    const className = document.getElementById('class').value || '';
    const section = document.getElementById('section').value || '';
    ...
    fetch(`{{ route('accounting.fee-document.decrement-amount.get-students') }}?${params.toString()}`)
        .then(response => response.json())
        .then(data => {
            ...
            students.forEach(student => {
                ...
                row.innerHTML = `
                    ...
                    <td class="new-fee">${currentFee.toFixed(2)}</td>
                `;
            ...
            // Only update preview if amount is already entered
...
function updateAmountPreview() {
    const amountInput = document.getElementById('amount');
    const decreaseValue = parseFloat(amountInput.value || 0);
    ...
    rows.forEach(row => {
        ...
        // Only update if amount is entered, otherwise show current fee
        if (decreaseValue > 0) {
            const newFee = Math.max(0, currentFee - decreaseValue);
            newFeeCell.textContent = newFee.toFixed(2);
        } else {
            newFeeCell.textContent = currentFee.toFixed(2);
        }
    });
}
