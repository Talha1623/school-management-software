...
@section('title', 'Decrement By Percentage')

@section('content')
<div class="row">
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3">
            <h3 class="mb-3 fw-semibold" style="color: #003471;">Decrement By Percentage</h3>
            ...
            <form id="feeDecrementPercentageForm" method="POST" action="{{ route('accounting.fee-document.decrement-percentage.store') }}">
                @csrf
                ...
                <div class="payment-row mb-3 p-3 border rounded" style="background-color: #f8f9fa;">
                    <div class="row g-3">
                        ...
                        <!-- Decrement -->
                        <div class="col-md-6">
                            <label class="form-label mb-1 fs-13 fw-medium">Decrement (%)</label>
                            <div class="input-group">
                                <span class="input-group-text" style="background-color: #f0f4ff; border-color: #e0e7ff;">
                                    <span class="material-symbols-outlined" style="font-size: 18px; color: #003471;">trending_down</span>
                                </span>
                                <input type="number" step="0.01" min="0" class="form-control form-control-sm" name="decrement" id="decrement" placeholder="Enter Decrement Percentage" required style="height: 38px;">
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadClassesByCampus(campus) {
        ...
        fetch(`{{ route('accounting.fee-document.decrement-percentage.get-classes-by-campus') }}?campus=${encodeURIComponent(campus)}`)
    ...
    function loadSectionsByClass(campus, className) {
        ...
        fetch(`{{ route('accounting.fee-document.decrement-percentage.get-sections-by-class') }}?${params.toString()}`)
            .then(response => response.json())
            .then(sections => {
                                    ...
                                    <label class="form-check-label w-100" for="student_${student.id}" style="cursor: pointer;">
                                        <div class="d-flex justify-content-between align-items-start">
                                            ...
                                            <div class="text-end ms-2">
                                                ...
                                                <div id="new_fee_${student.id}" class="text-danger fw-bold" style="font-size: 11px; display: none;"></div>
                                            </div>
                                        </div>
                                    </label>
                                </div>
