...
<div class="row">
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3">
            ...
            <form action="{{ route('accounting.generate-transport-fee.store') }}" method="POST" id="transport-fee-form">
                @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadClassesForCampus() {
        ...
        fetch(`{{ route('accounting.transport-fee.get-classes-by-campus') }}?campus=${encodeURIComponent(campus)}`, {
            headers: {
    ...
    function loadStudents() {
        ...
        fetch(`{{ route('accounting.transport-fee.get-students-with-fee-status') }}?${params.toString()}`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
            }
    ...
    classSelect.addEventListener('change', function() {
        ...
        fetch(`{{ route('accounting.transport-fee.get-sections-by-class') }}?class=${encodeURIComponent(selectedClass)}&campus=${encodeURIComponent(campus)}`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
            }
