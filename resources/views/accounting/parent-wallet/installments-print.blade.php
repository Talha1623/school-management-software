...
<div class="print-container">
  <div class="header">
    ...
    <div class="report-title">Fee Installments</div>
  </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th style="width:95px;">Fee Month</th>
        <th style="width:95px;">Installment Fee</th>
        <th style="width:95px;">Amount Paid</th>
      </tr>
    </thead>
    <tbody>
      @forelse($items as $i => $row)
        <tr>
          ...
          <td>{{ $row['class_section'] ?? 'N/A' }}</td>
          <td>{{ $row['installment_title'] ?? 'N/A' }}</td>
          <td>{{ $row['fee_month'] ?? 'N/A' }}</td>
          <td class="right">{{ number_format((float)($row['installment_fee'] ?? 0), 2) }}</td>
          <td class="right">{{ number_format((float)($row['amount_paid'] ?? 0), 2) }}</td>
        </tr>
      @empty
        <tr>
          <td colspan="8" class="center">No installments found</td>
        </tr>
      @endforelse
    </tbody>
  </table>
  ...
  <div class="footer">
    <div><strong>Total:</strong> {{ is_countable($items) ? count($items) : 0 }}</div>
