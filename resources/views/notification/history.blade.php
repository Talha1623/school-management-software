    ...
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3 mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="mb-0 fs-16 fw-semibold">Notification History</h4>
            </div>
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="input-group input-group-sm search-input-group" style="width: 280px;">
                            ...
                            <input type="text" id="searchInput" class="form-control border-start-0 border-end-0" placeholder="Search notification history..." value="{{ request('search') }}" onkeypress="handleSearchKeyPress(event)" oninput="handleSearchInput(event)" style="padding: 4px 8px; font-size: 13px;">
                            @if(request('search'))
            ...
            @if(request('search'))
                <div class="search-results-info">
                    ...
                    <a href="{{ route('notification.history') }}" class="text-decoration-none ms-2" style="color: #003471;">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @if(isset($paginator) && $paginator->count() > 0)
                                @forelse($paginator as $notification)
                                    <tr>
                                        <td>{{ $loop->iteration + (($paginator->currentPage() - 1) * $paginator->perPage()) }}</td>
                                        <td>{{ $notification->created_at ?? 'N/A' }}</td>
                                        <td>
                                            <span class="badge bg-info text-white">{{ $notification->recipient_type ?? 'N/A' }}</span>
                                        </td>
                                        <td>{{ $notification->title ?? 'N/A' }}</td>
                                        <td>{{ $notification->body ? (strlen($notification->body) > 50 ? substr($notification->body, 0, 50) . '...' : $notification->body) : 'N/A' }}</td>
                                        <td>{{ $notification->campus ?? 'N/A' }}</td>
                                        <td>
                                            <span class="badge bg-success text-white">{{ $notification->status ?? 'Sent' }}</span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
