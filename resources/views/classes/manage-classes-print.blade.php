...
<div class="print-container">
  <div class="header">
    ...
    <div class="report-title">Classes List</div>
  </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th>Class Name</th>
        <th style="width:90px;">Numeric No</th>
        <th style="width:220px;">Sections</th>
      </tr>
    </thead>
    <tbody>
      @forelse($classes as $i => $class)
        <tr>
          ...
          <td class="center">{{ $class->numeric_no ?? 'N/A' }}</td>
          <td>{{ !empty($class->sections) ? implode(', ', $class->sections) : 'No sections' }}</td>
        </tr>
      @empty
        <tr>
          <td colspan="5" class="center">No classes found</td>
        </tr>
      @endforelse
    </tbody>
  </table>
  ...
  <div class="footer">
    <div><strong>Total:</strong> {{ $classes->count() }}</div>
