    ...
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3 mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="mb-0 fs-16 fw-semibold">Loan Management</h4>
                <button type="button" class="btn btn-sm py-2 px-3 d-inline-flex align-items-center gap-1 rounded-8 loan-add-btn" data-bs-toggle="modal" data-bs-target="#loanModal" onclick="resetForm()">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-3 p-3 rounded-8" style="background-color: #f8f9fa; border: 1px solid #e9ecef;">
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="d-flex gap-2">
                        <a href="{{ route('salary-loan.loan-management.export', ['format' => 'excel']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn excel-btn">
                            <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">description</span>
                            <span>Excel</span>
                        </a>
                        <a href="{{ route('salary-loan.loan-management.export', ['format' => 'pdf']) }}{{ request()->has('search') ? '?search=' . request('search') : '' }}" class="btn btn-sm px-2 py-1 export-btn pdf-btn" target="_blank">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="input-group input-group-sm search-input-group" style="width: 280px;">
                            ...
                            <input type="text" id="searchInput" class="form-control border-start-0 border-end-0" placeholder="Search loans..." value="{{ request('search') }}" onkeypress="handleSearchKeyPress(event)" oninput="handleSearchInput(event)" style="padding: 4px 8px; font-size: 13px;">
                            @if(request('search'))
            ...
            @if(request('search'))
                <div class="search-results-info">
                    ...
                    @if(isset($loans))
                        ({{ $loans->total() }} {{ Str::plural('result', $loans->total()) }} found)
                    @endif
                    <a href="{{ route('salary-loan.loan-management') }}" class="text-decoration-none ms-2" style="color: #003471;">
                            ...
                            @if(isset($loans) && $loans->count() > 0)
                                @forelse($loans as $loan)
                                    <tr>
                                        ...
                                        <td>
                                            <strong class="text-primary">{{ $loan->staff->name ?? 'N/A' }}</strong>
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-end">
                                            <div class="d-inline-flex gap-1">
                                                <button type="button" class="btn btn-sm btn-primary px-2 py-0" title="Edit" onclick="editLoan({{ $loan->id }}, {{ $loan->staff_id }}, {{ $loan->requested_amount }}, {{ $loan->approved_amount ?? 'null' }}, {{ $loan->repayment_instalments }}, '{{ $loan->status }}')">
                                                    <span class="material-symbols-outlined" style="font-size: 14px; color: white;">edit</span>
                                                </button>
                                                <button type="button" class="btn btn-sm btn-danger px-2 py-0" title="Delete" onclick="if(confirm('Are you sure you want to delete this loan application?')) { document.getElementById('delete-form-{{ $loan->id }}').submit(); }">
                                                    <span class="material-symbols-outlined" style="font-size: 14px; color: white;">delete</span>
                                                </button>
                                                <form id="delete-form-{{ $loan->id }}" action="{{ route('salary-loan.loan-management.destroy', $loan->id) }}" method="POST" class="d-none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px; overflow: hidden;">
            <div class="modal-header text-white p-3" style="background: linear-gradient(135deg, #003471 0%, #004a9f 100%); border: none;">
                <h5 class="modal-title fs-15 fw-semibold mb-0 d-flex align-items-center gap-2" id="loanModalLabel">
                    <span class="material-symbols-outlined" style="font-size: 20px;">account_balance</span>
                    <span>Add New Application</span>
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" style="opacity: 0.8;"></button>
            </div>
            <form id="loanForm" method="POST" action="{{ route('salary-loan.loan-management.store') }}">
                @csrf
                <div id="methodField"></div>
                <div class="modal-body p-3">
                    <div class="row g-2">
                        <div class="col-md-12">
                            <label class="form-label mb-1 fs-12 fw-semibold" style="color: #003471;">For Teacher <span class="text-danger">*</span></label>
                            <div class="input-group input-group-sm loan-input-group">
                                ...
                                <select class="form-control loan-input" name="staff_id" id="staff_id" required style="border: none; border-left: 1px solid #e0e7ff; border-radius: 0 8px 8px 0; height: 36px;">
                                    <option value="">Select Teacher</option>
                                    @if(isset($staff) && $staff->count() > 0)
                                        @foreach($staff as $teacher)
                                            <option value="{{ $teacher->id }}">{{ $teacher->name }} ({{ $teacher->designation ?? 'N/A' }})</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            <label class="form-label mb-1 fs-12 fw-semibold" style="color: #003471;">Requested Amount <span class="text-danger">*</span></label>
                            <div class="input-group input-group-sm loan-input-group">
                                <span class="input-group-text" style="background-color: #f0f4ff; border-color: #e0e7ff; color: #003471;">
                                    <span class="material-symbols-outlined" style="font-size: 15px;">currency_rupee</span>
                                </span>
                                <input type="number" class="form-control loan-input" name="requested_amount" id="requested_amount" placeholder="Enter requested amount" step="0.01" min="0" required style="height: 36px;">
                            </div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="input-group input-group-sm loan-input-group">
                                ...
                                <input type="number" class="form-control loan-input" name="approved_amount" id="approved_amount" placeholder="Enter approved amount" step="0.01" min="0" style="height: 36px;">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label mb-1 fs-12 fw-semibold" style="color: #003471;">Repayment Instalments <span class="text-danger">*</span></label>
                            <div class="input-group input-group-sm loan-input-group">
                                <span class="input-group-text" style="background-color: #f0f4ff; border-color: #e0e7ff; color: #003471;">
                                    <span class="material-symbols-outlined" style="font-size: 15px;">calendar_month</span>
                                </span>
                                <input type="number" class="form-control loan-input" name="repayment_instalments" id="repayment_instalments" placeholder="Enter instalments" min="1" required style="height: 36px;">
                            </div>
                        </div>
                        <div class="col-md-6" id="statusField" style="display: none;">
                            <label class="form-label mb-1 fs-12 fw-semibold" style="color: #003471;">Status <span class="text-danger">*</span></label>
                            <div class="input-group input-group-sm loan-input-group">
                                <span class="input-group-text" style="background-color: #f0f4ff; border-color: #e0e7ff; color: #003471;">
                ...
                <div class="modal-footer p-3" style="background-color: #f8f9fa; border-top: 1px solid #e9ecef;">
                    ...
                    <button type="submit" class="btn btn-sm py-2 px-4 rounded-8 loan-submit-btn">
...
<style>
    ...
    #loanModal .loan-input-group:focus-within {
        box-shadow: 0 0 0 3px rgba(0, 52, 113, 0.15);
        border-color: #003471;
    }
    
    #loanModal .loan-input {
        height: 36px;
        font-size: 13px;
    ...
    #loanModal .loan-input:focus {
        border-left-color: #003471;
        box-shadow: none;
        outline: none;
    }
    
    #loanModal .input-group-text {
        height: 36px;
        padding: 0 0.75rem;
        display: flex;
        align-items: center;
    ...
    #loanModal .loan-input-group:focus-within .input-group-text {
        background-color: #003471 !important;
        color: white !important;
        border-right-color: #003471;
    }
    
    #loanModal .form-label {
        margin-bottom: 0.4rem;
        line-height: 1.3;
    }
    
    #loanModal .loan-submit-btn {
    ...
    #loanModal .loan-submit-btn:hover {
        background: linear-gradient(135deg, #004a9f 0%, #003471 100%);
        transform: translateY(-1px);
        box-shadow: 0 4px 12px rgba(0, 52, 113, 0.35);
    }
    
    #loanModal .loan-submit-btn:active {
...
<script>
// Reset form when opening modal for new loan
function resetForm() {
    document.getElementById('loanForm').reset();
    document.getElementById('loanForm').action = "{{ route('salary-loan.loan-management.store') }}";
    document.getElementById('methodField').innerHTML = '';
    document.getElementById('statusField').style.display = 'none';
    document.getElementById('loanModalLabel').innerHTML = `
        <span class="material-symbols-outlined" style="font-size: 20px;">account_balance</span>
        <span>Add New Application</span>
    `;
    document.querySelector('.loan-submit-btn').innerHTML = `
        <span class="material-symbols-outlined" style="font-size: 16px; vertical-align: middle;">save</span>
        Save Application
    `;
}
...
// Edit loan function
function editLoan(id, staffId, requestedAmount, approvedAmount, repaymentInstalments, status) {
    document.getElementById('staff_id').value = staffId;
    document.getElementById('requested_amount').value = requestedAmount;
    document.getElementById('approved_amount').value = approvedAmount || '';
    document.getElementById('repayment_instalments').value = repaymentInstalments;
    document.getElementById('status').value = status;
    document.getElementById('loanForm').action = "{{ url('salary-loan/loan-management') }}/" + id;
