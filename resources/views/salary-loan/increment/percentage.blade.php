    ...
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3 mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="mb-0 fs-16 fw-semibold">Increment By Percentage</h4>
            </div>
            ...
            <form action="{{ route('salary-loan.increment.percentage.store') }}" method="POST" id="incrementPercentageForm" onsubmit="return validateForm()">
                @csrf
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <div class="card bg-light border-0 rounded-10 p-3 mb-2">
                            ...
                            <label for="increase" class="form-label mb-1 fs-12 fw-semibold" style="color: #003471;">Salary Increase (%) <span class="text-danger">*</span></label>
                            <div class="input-group input-group-sm increment-input-group">
                                <span class="input-group-text" style="background-color: #f0f4ff; border-color: #e0e7ff; color: #003471;">
                                    <span class="material-symbols-outlined" style="font-size: 15px;">trending_up</span>
                                </span>
                                <input type="number" class="form-control increment-input" name="increase" id="increase" placeholder="Enter percentage" step="0.01" min="0" max="100" required style="height: 36px;">
                    ...
                    <div class="col-md-6" id="feesIncrementContainer" style="display: none;">
                        <div class="card bg-light border-0 rounded-10 p-3 mb-2">
                            ...
                            <div class="input-group input-group-sm increment-input-group mb-2">
                                ...
                                <input type="number" class="form-control increment-input" name="absent_fees_increment" id="absent_fees_increment" placeholder="Enter percentage (optional)" step="0.01" min="0" max="100" style="height: 36px;">
                                <span class="input-group-text" style="background-color: #f0f4ff; border-color: #e0e7ff; color: #003471;">%</span>
                            </div>
                            ...
                            <div class="input-group input-group-sm increment-input-group mb-2">
                                ...
                                <input type="number" class="form-control increment-input" name="late_fees_increment" id="late_fees_increment" placeholder="Enter percentage (optional)" step="0.01" min="0" max="100" style="height: 36px;">
                                <span class="input-group-text" style="background-color: #f0f4ff; border-color: #e0e7ff; color: #003471;">%</span>
                            </div>
                            ...
                            <div class="input-group input-group-sm increment-input-group">
                                ...
                                <input type="number" class="form-control increment-input" name="early_exit_fees_increment" id="early_exit_fees_increment" placeholder="Enter percentage (optional)" step="0.01" min="0" max="100" style="height: 36px;">
                                <span class="input-group-text" style="background-color: #f0f4ff; border-color: #e0e7ff; color: #003471;">%</span>
                            </div>
                            <small class="text-muted" style="font-size: 10px;">Only apply if percentage is entered, otherwise fees will not be incremented</small>
                        </div>
                    </div>
...
// Reset form
function resetForm() {
    document.getElementById('incrementPercentageForm').reset();
...
function loadStaffByCampus() {
    ...
    fetch(`{{ route('salary-loan.increment.percentage.get-staff-by-campus') }}?campus=${encodeURIComponent(campus)}&salary_type=${encodeURIComponent(salaryType)}`, {
        headers: {
...
// Update salary preview when percentage changes
document.getElementById('increase').addEventListener('input', function() {
    updateSalaryPreview();
});

// Update preview when fees increment fields change
const absentFeesIncrementField = document.getElementById('absent_fees_increment');
const lateFeesIncrementField = document.getElementById('late_fees_increment');
const earlyExitFeesIncrementField = document.getElementById('early_exit_fees_increment');

if (absentFeesIncrementField) {
    absentFeesIncrementField.addEventListener('input', function() {
        updateSalaryPreview();
    });
}
if (lateFeesIncrementField) {
    lateFeesIncrementField.addEventListener('input', function() {
        updateSalaryPreview();
    });
}
if (earlyExitFeesIncrementField) {
    earlyExitFeesIncrementField.addEventListener('input', function() {
        updateSalaryPreview();
    });
}
...
function updateSalaryPreview() {
    const percentage = parseFloat(document.getElementById('increase').value || 0);
    ...
    if (percentage > 0) {
        checkboxes.forEach(checkbox => {
            ...
            // Update fees preview - only show if manual increment percentage is provided
            ...
            if (absentFeesSpan && absentFeesIncrement !== null && absentFeesIncrement > 0) {
                ...
                if (originalAbsent > 0) {
                    const newAbsent = originalAbsent + (originalAbsent * (absentFeesIncrement / 100));
            ...
            if (lateFeesSpan && lateFeesIncrement !== null && lateFeesIncrement > 0) {
                ...
                if (originalLate > 0) {
                    const newLate = originalLate + (originalLate * (lateFeesIncrement / 100));
            ...
            if (earlyExitFeesSpan && earlyExitFeesIncrement !== null && earlyExitFeesIncrement > 0) {
                ...
                if (originalEarlyExit > 0) {
                    const newEarlyExit = originalEarlyExit + (originalEarlyExit * (earlyExitFeesIncrement / 100));
