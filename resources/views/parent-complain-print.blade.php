...
<div class="print-container">
  <div class="header">
    ...
    <div class="report-title">Parent Complains List</div>
  </div>
    ...
    <tbody>
      @forelse($complains as $i => $c)
        <tr>
          ...
          <td>{{ $c->parent_name ?? 'N/A' }}</td>
          <td>{{ $c->email ?? 'N/A' }}</td>
          <td>{{ $c->phone ?? 'N/A' }}</td>
          <td>{{ $c->subject ?? 'N/A' }}</td>
          <td>{{ $c->complain ?? 'N/A' }}</td>
          <td>{{ $c->created_at ? $c->created_at->format('d M Y') : 'N/A' }}</td>
          <td>{{ $c->reply ? 'Replied' : 'Pending' }}</td>
        </tr>
      @empty
        <tr>
          <td colspan="8" class="center">No complains found</td>
        </tr>
      @endforelse
    </tbody>
  </table>
  ...
  <div class="footer">
    <div><strong>Total:</strong> {{ $complains->count() }}</div>
