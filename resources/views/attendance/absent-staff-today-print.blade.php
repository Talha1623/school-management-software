...
<head>
    ...
    <title>Absent Staff Today</title>
    <style>
...
<div class="print-container">
    <div class="header-section">
        ...
        <div class="report-title">Absent Staff Today</div>
    ...
    <div class="table-wrap">
        <table>
            ...
            <tr>
                ...
                <th style="width:9%;">Father / Husband</th>
            ...
            @forelse($records as $index => $attendance)
                @php
                    ...
                    $phone = $staff->phone ?? $staff->whatsapp ?? 'N/A';
                @endphp
                <tr>
                    <td>{{ $index + 1 }}</td>
                    ...
                    <td>{{ $phone }}</td>
                    <td>{{ $attendance->campus ?? $staff->campus ?? 'N/A' }}</td>
                    <td>{{ $attendance->designation ?? $staff->designation ?? 'N/A' }}</td>
                    <td>{{ $attendance->remarks ?? '—' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="8" style="text-align:center;color:#6b7280;">No absent staff found for this date.</td>
                </tr>
            @endforelse
            @if($records->count() > 0)
                <tr>
                    <td colspan="8" class="num" style="font-weight:700;background:#f3f4f6;">Total absent: {{ $records->count() }}</td>
                </tr>
            @endif
            </tbody>
        </table>
