                            ...
                            <div class="d-xxl-flex align-items-center bg-transparent border-0 text-start p-0 cursor dropdown-toggle" data-bs-toggle="dropdown">
                                <div class="flex-shrink-0 position-relative">
                                    @if(Auth::guard('student')->check() && Auth::guard('student')->user()->photo)
                                        <img class="rounded-circle admin-img-width-for-mobile" style="width: 40px; height: 40px; object-fit: cover;" src="{{ Storage::url(Auth::guard('student')->user()->photo) }}" alt="student">
                                    @else
                                        <img class="rounded-circle admin-img-width-for-mobile" style="width: 40px; height: 40px;" src="{{ asset('assets/images/admin.png') }}" alt="student">
                                    @endif
                                ...
                                <div class="d-flex align-items-center info">
                                    ...
                                    <div class="flex-grow-1 ms-10">
                                        @if(Auth::guard('student')->check())
                                            <h3 class="fw-medium fs-17 mb-0">{{ Auth::guard('student')->user()->student_name }}</h3>
                                            <span class="fs-15 fw-medium">{{ Auth::guard('student')->user()->student_code ?? 'Student' }}</span>
                                        @endif
                                    </div>
                                </div>
                                <ul class="admin-link mb-0 list-unstyled">
                                    <li>
                                        <a class="dropdown-item admin-item-link d-flex align-items-center text-body" href="{{ route('student.dashboard') }}">
...
function handleLogout() {
    var studentForm = document.getElementById('student-logout-form');
    if (studentForm) {
        studentForm.submit();
        return;
    }
