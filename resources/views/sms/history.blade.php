    ...
    <div class="col-12">
        <div class="card bg-white border border-white rounded-10 p-3 mb-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="mb-0 fs-16 fw-semibold">SMS History</h4>
            </div>
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="input-group input-group-sm search-input-group" style="width: 280px;">
                            ...
                            <input type="text" id="searchInput" class="form-control border-start-0 border-end-0" placeholder="Search SMS history..." value="{{ request('search') }}" onkeypress="handleSearchKeyPress(event)" oninput="handleSearchInput(event)" style="padding: 4px 8px; font-size: 13px;">
                            @if(request('search'))
            ...
            @if(request('search'))
                <div class="search-results-info">
                    ...
                    <a href="{{ route('sms.history') }}" class="text-decoration-none ms-2" style="color: #003471;">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @if(isset($paginator) && $paginator->count() > 0)
                                @forelse($paginator as $sms)
                                    <tr>
                                        <td>{{ $loop->iteration + (($paginator->currentPage() - 1) * $paginator->perPage()) }}</td>
                                        <td>{{ $sms->created_at ?? 'N/A' }}</td>
                                        <td>
                                            <span class="badge bg-info text-white">{{ $sms->recipient_type ?? 'N/A' }}</span>
                                        </td>
                                        <td>{{ $sms->recipient ?? 'N/A' }}</td>
                                        <td>{{ $sms->message ? (strlen($sms->message) > 50 ? substr($sms->message, 0, 50) . '...' : $sms->message) : 'N/A' }}</td>
                                        <td>
                                            <span class="badge bg-success text-white">{{ $sms->status ?? 'Sent' }}</span>
                                        </td>
                                        <td>{{ $sms->characters ?? strlen($sms->message ?? '') }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="7" class="text-center text-muted py-5">
                                            <span class="material-symbols-outlined" style="font-size: 48px; opacity: 0.3;">inbox</span>
                                            <p class="mt-2 mb-0">No SMS history found.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            @else
