...
<head>
    ...
    <title>Transport Routes Print</title>
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .header-section { border-bottom: 3px solid #003471; padding-bottom: 10px; }
        .school-name { font-size: 22px; font-weight: 800; color: #003471; }
...
<div class="print-container">
    <div class="header-section">
        ...
        <div class="report-title">Transport Routes List</div>
        <div class="meta-row">
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Route Name</th>
                <th style="width:180px;">Number Of Vehicle</th>
                <th>Description</th>
                <th style="width:120px;">Route Fare</th>
            </tr>
        </thead>
        <tbody>
        @forelse($transports as $i => $transport)
            <tr>
                <td>{{ $i + 1 }}</td>
                <td>{{ $transport->campus ?? 'N/A' }}</td>
                <td><strong>{{ $transport->route_name ?? 'N/A' }}</strong></td>
                <td>{{ $transport->number_of_vehicle ?? 'N/A' }}</td>
                <td>{{ $transport->description ?? 'N/A' }}</td>
                <td class="text-end">{{ is_numeric($transport->route_fare) ? number_format($transport->route_fare, 2) : $transport->route_fare }}</td>
            </tr>
        @empty
            <tr>
                <td colspan="6" style="text-align:center; color:#6b7280;">No transport routes found.</td>
            </tr>
        @endforelse
        </tbody>
    </table>
    ...
    <div class="footer-section">
        <div><strong>Total:</strong> {{ $transports->count() }}</div>
