...
<title>Manage Inquiry List</title>
...
<tr>
    ...
    <td>{{ $inq->parent }}</td>
    <td>{{ $inq->phone }}</td>
    <td>{{ ucfirst($inq->gender) }}</td>
    <td>{{ $inq->birthday ? $inq->birthday->format('d M Y') : 'N/A' }}</td>
    <td>{{ $inq->full_address }}</td>
    <td>{{ $inq->created_at ? $inq->created_at->format('d M Y') : 'N/A' }}</td>
</tr>
@empty
<tr>
    <td colspan="8" class="center">No inquiries found</td>
</tr>
@endforelse
</tbody>
</table>
